...
export class World {
  hexagonVertexRadius; // How many side lengths of terrain a hexagon is worth (vertices -1).
  innerHexagonVertexRadius; // How many side lengths from inner hexagon edge to center. (vertices -1)

  terrainWidth; // Worldspace size of terrain
  terrainHeight;
  terrainVertexWidth; // How many side lengths of terrain in the entire terrain (vertices - 1).
  terrainVertexHeight;

  tileGridWidth; // How many hexagons horizontally
  tileGridHeight;
  ...
  constructor(tileGridWidth, tileGridHeight, hexagonWorldRadius) {
  ...
  makeTerrain() {
    let points = new Array(this.terrainVertexWidth*this.terrainVertexHeight);
  ...
  addUpTriangleToArray(x,y,z,
                       onePointLen, halfPointLen, onePointHeight, arr) {
